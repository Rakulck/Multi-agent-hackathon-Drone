 ...
 */
export function MapFallback({ reason, routeStatuses, selectedRoute, hazardVisible, dronePosition, dropOffZone, scene, onRetry }: MapFallbackProps) {
  ...
  return (
    <div className="relative flex h-full min-h-0 items-center justify-center overflow-hidden rounded-[28px] bg-white">
      ...
      <div className="absolute bottom-4 left-4 rounded-full border border-neutral-200 bg-white/80 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-neutral-500 backdrop-blur">
        {SCENE_LABEL}
      </div>
    </div>
  );
...
function projectionBoundsForScene(scene: MissionMapScene): ProjectionBounds {
  const lats = [scene.origin.lat, scene.destination.lat, scene.hazard.center.lat];
  const lngs = [scene.origin.lng, scene.destination.lng, scene.hazard.center.lng];
